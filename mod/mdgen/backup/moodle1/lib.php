...
/**
 * mdgen conversion handler. This resource handler is called by moodle1_mod_resource_handler
 */
class moodle1_mod_mdgen_handler extends moodle1_resource_successor_handler {
    ...
    public function process_legacy_resource(array $data, array $raw = null) {
        ...
        // convert the legacy data onto the new mdgen record
        $page                       = array();
        $page['id']                 = $data['id'];
        $page['name']               = $data['name'];
        ...
        if ($data['type'] === 'html') {
            // legacy Resource of the type Web mdgen
            $page['contentformat'] = FORMAT_HTML;

        } else {
            // legacy Resource of the type Plain text mdgen
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_mdgen');
        ...
        // write mdgen.xml
        $this->open_xml_writer("activities/mdgen_{$moduleid}/mdgen.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $moduleid,
            'modulename' => 'mdgen', 'contextid' => $contextid));
        $this->write_xml('mdgen', $page, array('/mdgen/id'));
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // write inforef.xml for migrated resource file.
        $this->open_xml_writer("activities/mdgen_{$moduleid}/inforef.xml");
