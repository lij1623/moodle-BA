...
namespace editor_marklar2\privacy;
...
class provider implements
    ...
    public static function _get_metadata(collection $collection) {
        $collection->add_user_preference('editor_marklar2/formats', 'preferencesformat');
        return $collection;
    }
    ...
    public static function _export_user_preferences($userid) {
        ...
        $raw = (array) json_decode(get_user_preferences('editor_marklar2/formats', null, $userid));
        ...
        if (is_array($raw)) {
            foreach ($raw as $format => $enabled) {
                ...
                $desc = get_string('privacy:export:preferences:format', 'editor_marklar2', ['format' => $name]);

                writer::export_user_preference('editor_marklar2', 'formats.'.$format, transform::yesno($enabled), $desc);
            }
        }
    }
}
