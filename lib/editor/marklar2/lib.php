 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class marklar2_texteditor extends texteditor {
    ...
    public function use_editor($elementid, array $options=null, $fpoptions = null) {
        ...
        $PAGE->requires->js_call_amd('editor_marklar2/editor', 'init', [$initparams]);
        ...
        $PAGE->requires->js_init_code('M.editor_marklar2 = M.editor_marklar2 || {}');
        $PAGE->requires->js_init_code('M.editor_marklar2.fpoptions = M.editor_marklar2.fpoptions || {}');
        $PAGE->requires->js_init_code(js_writer::set_variable('M.editor_marklar2.fpoptions.'.$elementid, $fpoptions));
    }
}
 ...
 */
function editor_marklar2_extend_navigation_user_settings(navigation_node $usersetting, $user, context_user $usercontext,
    ...
    if (empty($preference) and !empty($CFG->texteditors)) {
        ...
        if (reset($editors) === 'marklar2') {
            $preference = 'marklar2';
        }
    }

    // If the user's preferred editor is Marklar, show a link to Marklar preferences page.
    if ($preference === 'marklar2') {
        $prefurl = new moodle_url('/lib/editor/marklar2/preferences.php', ['userid' => $user->id]);
        $s = get_string('preferences', 'editor_marklar2');
        $usersetting->add($s, $prefurl);
    }
}
