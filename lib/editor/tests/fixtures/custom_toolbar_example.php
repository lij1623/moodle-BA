...
/**
 * Demonstrates use of newedit editor with overridden toolbar setting.
 *
...
require_once($CFG->dirroot . '/lib/editor/newedit/lib.php');

// Behat test fixture only.
defined('BEHAT_SITE_RUNNING') || die('Only available on Behat test server');

$PAGE->set_url('/lib/editor/newedit/tests/fixtures/override_plugins_example.php');
...
if ($normal !== '' || $special !== '') {
    ...
    $options = array();
    $newedit = new newedit_texteditor();

    // Normal newedit.
    echo html_writer::start_div('normaldiv');
    echo $OUTPUT->heading('Normal newedit');
    echo html_writer::div(html_writer::tag('textarea', '',
            array('id' => 'normaleditor', 'name' => 'normaleditor', 'rows' => 10)));
    $newedit->use_editor('normaleditor', $options);
    echo html_writer::end_div();

    // Second newedit with custom options.
    echo html_writer::start_div('specialdiv');
    $options['newedit:toolbar'] = <<<EOT
style1 = bold, italic
list = unorderedlist, orderedlist
EOT;
    echo $OUTPUT->heading('Special newedit');
    echo html_writer::div(html_writer::tag('textarea', '',
            array('id' => 'specialeditor', 'name' => 'specialeditor', 'rows' => 10)));
    $newedit->use_editor('specialeditor', $options);
    echo html_writer::end_div();
