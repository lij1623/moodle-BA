YUI.add('moodle-mde_recordrtc-button', function (Y, NAME) {
...
/**
 * @module moodle-mde_recordrtc-button
 */

/**
 * mde text editor recordrtc plugin.
 *
 * @namespace M.mde_recordrtc
 * @class button
...
Y.namespace('M.mde_recordrtc').Button = Y.Base.create('button', Y.M.editor_mde.EditorPlugin, [], {
    /**
        ...
        if (this.get('host').canShowFilepicker('media')) {
            ...
            dialogue.after('visibleChange', function() {
                var closed = !dialogue.get('visible'),
                    m = M.mde_recordrtc.commonmodule;
    ...
    /**
     * Add the buttons to the mde toolbar.
     *
     * @method _addButton
     * @param {string} type
    ...
    _audio: function() {
        ...
        dialogue.set('headerContent', M.util.get_string('audiortc', 'mde_recordrtc'));
        dialogue.set('bodyContent', this._createContent('audio'));

        dialogue.show();

        M.mde_recordrtc.audiomodule.init(this);
    },
    ...
    _video: function() {
        ...
        dialogue.set('headerContent', M.util.get_string('videortc', 'mde_recordrtc'));
        dialogue.set('bodyContent', this._createContent('video'));

        dialogue.show();

        M.mde_recordrtc.videomodule.init(this);
    },
    ...
    _createContent: function(type) {
        ...
        var bodyContent = Y.Handlebars.compile(TEMPLATE)({
            ...
            bs_al_dang: bsAlDang,
            bs_ss_btn: bsSsBtn,
            insecurealert_title: M.util.get_string('insecurealert_title', 'mde_recordrtc'),
            insecurealert: M.util.get_string('insecurealert', 'mde_recordrtc'),
            startrecording: M.util.get_string('startrecording', 'mde_recordrtc'),
            attachrecording: M.util.get_string('attachrecording', 'mde_recordrtc')
        });
...
}, '@VERSION@', {"requires": ["moodle-editor_mde-plugin", "moodle-mde_recordrtc-recording"]});
