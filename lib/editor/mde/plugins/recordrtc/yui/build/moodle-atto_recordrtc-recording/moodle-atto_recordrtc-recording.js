YUI.add('moodle-mde_recordrtc-recording', function (Y, NAME) {
...
M.mde_recordrtc = M.mde_recordrtc || {};

// Shorten access to M.mde_recordrtc.commonmodule namespace.
var cm = M.mde_recordrtc.commonmodule,
    am = M.mde_recordrtc.abstractmodule;

M.mde_recordrtc.commonmodule = {
    ...
    handle_stop: function() {
        ...
        cm.uploadBtn.set('textContent', M.util.get_string('attachrecording', 'mde_recordrtc'));
                ...
                cm.upload_to_server(cm.recType, function(progress, fileURLOrError) {
                    if (progress === 'ended') { // Insert annotation in text.
                        ...
                        cm.uploadBtn.set('textContent',
                            M.util.get_string('uploadfailed', 'mde_recordrtc') + ' ' + fileURLOrError);
                    } else if (progress === 'upload-failed-404') { // 404 error = File too large in Moodle.
                        cm.uploadBtn.set('disabled', false);
                        cm.uploadBtn.set('textContent', M.util.get_string('uploadfailed404', 'mde_recordrtc'));
                    } else if (progress === 'upload-aborted') {
                        cm.uploadBtn.set('disabled', false);
                        cm.uploadBtn.set('textContent',
                            M.util.get_string('uploadaborted', 'mde_recordrtc') + ' ' + fileURLOrError);
                    } else {
    ...
    start_recording: function(type, stream) {
        ...
        cm.countdownSeconds++;
        var timerText = M.util.get_string('stoprecording', 'mde_recordrtc');
    ...
    make_xmlhttprequest: function(url, data, callback) {
        ...
        xhr.upload.onprogress = function(event) {
            callback(Math.round(event.loaded / event.total * 100) + "% " + M.util.get_string('uploadprogress', 'mde_recordrtc'));
        };
...
/**
 * mde recordrtc library functions for checking browser compatibility
 *
 * @package    mde_recordrtc
 * @author     Jesus Federico (jesus [at] blindsidenetworks [dt] com)
 * @author     Jacob Prud'homme (jacob [dt] prudhomme [at] blindsidenetworks [dt] com)
...
M.mde_recordrtc.compatcheckmodule = {
...
/**
 * mde recordrtc library functions for function abstractions
 *
 * @package    mde_recordrtc
 * @author     Jesus Federico (jesus [at] blindsidenetworks [dt] com)
 * @author     Jacob Prud'homme (jacob [dt] prudhomme [at] blindsidenetworks [dt] com)
...
M.mde_recordrtc.abstractmodule = {
    ...
    show_alert: function(subject, onCloseEvent) {
        Y.use('moodle-core-notification-alert', function() {
            var dialogue = new M.core.alert({
                title: M.util.get_string(subject + '_title', 'mde_recordrtc'),
                message: M.util.get_string(subject, 'mde_recordrtc')
            });
    ...
    handle_gum_errors: function(error, commonConfig) {
        var btnLabel = M.util.get_string('recordingfailed', 'mde_recordrtc'),
...
var cm = M.mde_recordrtc.commonmodule,
    am = M.mde_recordrtc.abstractmodule,
    ccm = M.mde_recordrtc.compatcheckmodule;

M.mde_recordrtc.audiomodule = {
    init: function(scope) {
        ...
        cm.startStopBtn.on('click', function() {
            ...
            if ((cm.startStopBtn.get('textContent') === M.util.get_string('startrecording', 'mde_recordrtc')) ||
                (cm.startStopBtn.get('textContent') === M.util.get_string('recordagain', 'mde_recordrtc')) ||
                (cm.startStopBtn.get('textContent') === M.util.get_string('recordingfailed', 'mde_recordrtc'))) {
                ...
                M.mde_recordrtc.audiomodule.capture_audio(commonConfig);
                ...
                cm.startStopBtn.set('textContent', M.util.get_string('recordagain', 'mde_recordrtc'));
                cm.startStopBtn.replaceClass('btn-danger', 'btn-outline-danger');
            }
    ...
    init: function(scope) {
        ...
        cm.startStopBtn.on('click', function() {
            ...
            if ((cm.startStopBtn.get('textContent') === M.util.get_string('startrecording', 'mde_recordrtc')) ||
                ...
                M.mde_recordrtc.videomodule.capture_audio_video(commonConfig);
            } else { // If button is displaying "Stop Recording".
                // First of all clears the countdownTicker.
                window.clearInterval(cm.countdownTicker);
...
}, '@VERSION@', {"requires": ["moodle-mde_recordrtc-button"]});
