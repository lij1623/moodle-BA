...
/**
 * Unit tests for the editor_mde implementation of the privacy API.
 *
 * @package    editor_mde
 * @category   test
 ...
 */
namespace editor_mde\privacy;

defined('MOODLE_INTERNAL') || die();

use core_privacy\local\request\writer;
use core_privacy\local\request\approved_contextlist;
use editor_mde\privacy\provider;
...
class provider_test extends \core_privacy\tests\provider_testcase {
    ...
    public function test_fetch_and_exports_drafts() {
        ...
        // There should be two.
        $this->export_context_data_for_user($user->id, $systemcontext, 'editor_mde');
        $writer = \core_privacy\local\request\writer::with_context($systemcontext);
        $this->assertTrue($writer->has_any_data());

        $subcontextbase = [get_string('autosaves', 'editor_mde')];
    ...
    public function test_delete_for_all_users_in_context() {
        ...
        $this->assertCount(2, $DB->get_records('editor_mde_autosave', ['contextid' => $usercontext->id]));
        provider::delete_data_for_all_users_in_context($usercontext);
        $this->assertCount(0, $DB->get_records('editor_mde_autosave', ['contextid' => $usercontext->id]));

        // No other contexts should be removed.
        $this->assertCount(2, $DB->get_records('editor_mde_autosave', ['contextid' => $otherusercontext->id]));
        $this->assertCount(4, $DB->get_records('editor_mde_autosave', ['contextid' => $systemcontext->id]));
        $this->assertCount(4, $DB->get_records('editor_mde_autosave', ['contextid' => $coursecontext->id]));

        // Test deletion of the course contexts.
        provider::delete_data_for_all_users_in_context($coursecontext);
        $this->assertCount(0, $DB->get_records('editor_mde_autosave', ['contextid' => $coursecontext->id]));
        $this->assertCount(2, $DB->get_records('editor_mde_autosave', ['contextid' => $otherusercontext->id]));
        $this->assertCount(4, $DB->get_records('editor_mde_autosave', ['contextid' => $systemcontext->id]));

        // Test deletion of the system contexts.
        provider::delete_data_for_all_users_in_context($systemcontext);
        $this->assertCount(0, $DB->get_records('editor_mde_autosave', ['contextid' => $systemcontext->id]));
        $this->assertCount(2, $DB->get_records('editor_mde_autosave', ['contextid' => $otherusercontext->id]));
    }
    ...
    public function test_delete_for_user_in_contexts() {
        ...
        $this->assertCount(0, $DB->get_records('editor_mde_autosave', ['contextid' => $usercontext->id]));
        $this->assertCount(2, $DB->get_records('editor_mde_autosave', ['contextid' => $otherusercontext->id]));
        $this->assertCount(2, $DB->get_records('editor_mde_autosave', ['contextid' => $systemcontext->id]));
        $this->assertCount(2, $DB->get_records('editor_mde_autosave', ['contextid' => $coursecontext->id]));

        // Data for the other user should remain.
        $this->assertCount(2, $DB->get_records('editor_mde_autosave', [
    ...
    public function test_another_user_edits_you() {
        ...
        $this->export_context_data_for_user($user->id, $usercontext, 'editor_mde');
        $writer = \core_privacy\local\request\writer::with_context($usercontext);
        $this->assertTrue($writer->has_any_data());

        $subcontext = [
            get_string('autosaves', 'editor_mde'),
            $userdraftintro->id,
        ];
        ...
        provider::delete_data_for_user($contextlist);
        $records = $DB->get_records('editor_mde_autosave');
    ...
    public function test_another_you_edit_different_user() {
        ...
        // Should have the data.
        $this->export_context_data_for_user($user->id, $otherusercontext, 'editor_mde');
        ...
        provider::delete_data_for_user($contextlist);
        $this->assertEmpty($DB->get_records('editor_mde_autosave'));
    }
   ...
   /**
    ...
    protected function create_editor_draft(\context $context, $userid, $elementid, $text, $filecount = 0) {
        ...
        $id = $DB->insert_record('editor_mde_autosave', (object) [
                'elementid' => $elementid,
                'contextid' => $context->id,
                'userid' => $userid,
                'drafttext' => $text,
        ...
        return $DB->get_record('editor_mde_autosave', ['id' => $id]);
    }
}
