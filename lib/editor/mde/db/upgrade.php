...
/**
 * Run all mde upgrade steps between the current DB version and the current version on disk.
 * @param int $oldversion The old version of mde in the DB.
 * @return bool
 */
function xmldb_editor_mde_upgrade($oldversion) {
    global $CFG;
    ...
    if ($oldversion < 2019090900) {
        $toolbar = get_config('editor_mde', 'toolbar');
        ...
        if (strpos($toolbar, 'h5p') === false) {
            ...
            foreach ($groups as $i => $group) {
                ...
                if (trim($parts[0]) == 'files') {
                    ...
                    set_config('toolbar', $toolbar, 'editor_mde');
                }
            }
        }
        // mde editor savepoint reached.
        upgrade_plugin_savepoint(true, 2019090900, 'editor', 'mde');
    }
...
other = html';
        ...
        if (get_config('editor_mde', 'toolbar') === $oldtoolbar) {
...
accessibility = accessibilitychecker, accessibilityhelper
other = html';
            set_config('toolbar', $newtoolbar, 'editor_mde');
        }

        upgrade_plugin_savepoint(true, 2020052100, 'editor', 'mde');
    }
...
other = html';
            set_config('toolbar', $newtoolbar, 'editor_mde');
            set_config('showgroups', 6, 'mde_collapse');
        }

        upgrade_plugin_savepoint(true, 2021062400, 'editor', 'mde');
    }
