 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mde_texteditor extends texteditor {
     ...
     *
     * Available mde-specific options:
     *   mde:toolbar - set to a string to override the system config editor_mde/toolbar
     *
    ...
    public function use_editor($elementid, array $options=null, $fpoptions=null) {
        ...
        if (array_key_exists('mde:toolbar', $options)) {
            $configstr = $options['mde:toolbar'];
        } else {
            $configstr = get_config('editor_mde', 'toolbar');
        }
        ...
        $modules = array('moodle-editor_mde-editor');
        ...
        foreach ($groups as $group => $plugins) {
            ...
            foreach ($plugins as $plugin) {
                ...
                if (!core_component::get_component_directory('mde_' . $plugin))  {
                    continue;
                }
                ...
                $modules[] = 'moodle-mde_' . $plugin . '-button';

                component_callback('mde_' . $plugin, 'strings_for_js');
                $extra = component_callback('mde_' . $plugin, 'params_for_js', array($elementid, $options, $fpoptions));

                if ($extra) {
                    $jsplugin = array_merge($jsplugin, $extra);
                }
                // We always need the plugin name.
                $PAGE->requires->string_for_js('pluginname', 'mde_' . $plugin);
    ...
    protected function get_init_params($elementid, array $options = null, array $fpoptions = null, $plugins = null) {
        ...
        $autosave       = true;
        $autosavefrequency = get_config('editor_mde', 'autosavefrequency');
