YUI.add('moodle-editor_mde-plugin', function (Y, NAME) {
 ...
 * mde editor plugin.
 *
 * @module moodle-editor_mde-plugin
...
/**
 * A Plugin for the mde Editor used in Moodle.
 *
 ...
 * @main
 * @constructor
 * @uses M.editor_mde.EditorPluginButtons
 * @uses M.editor_mde.EditorPluginDialogue
 */
...
Y.extend(EditorPlugin, Y.Base, {
     ...
     * {{#crossLink "M.editor_mde.EditorTextArea/updateOriginal"}}updateOriginal{{/crossLink}}.
     *
     * @method markUpdated
     */
    ...
    ATTRS: {
        ...
        group: {
            ...
            getter: function(groupName) {
                ...
                if (!group) {
                    group = Y.Node.create('<div class="mde_group ' +
                            groupName + GROUP + '"></div>');
...
Y.namespace('M.editor_mde').EditorPlugin = EditorPlugin;
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
...
/**
 * Button functions for an mde Plugin.
 *
 * See {{#crossLink "M.editor_mde.EditorPlugin"}}{{/crossLink}} for details.
 *
 * @namespace M.editor_mde
...
var MENUTEMPLATE = '' +
        '<button class="{{buttonClass}} mde_hasmenu" ' +
            'id="{{id}}" ' +
            'tabindex="-1" ' +
            'title="{{title}}" ' +
            'aria-label="{{title}}" ' +
            ...
            '<span class="editor_mde_menu_icon"></span>' +
            '<span class="editor_mde_menu_expand"></span>' +
        '</button>';

var DISABLED = 'disabled',
    HIGHLIGHT = 'highlight',
    LOGNAME = 'moodle-editor_mde-editor-plugin',
    CSS = {
        EDITORWRAPPER: '.editor_mde_content',
        MENUICON: '.editor_mde_menu_icon',
        MENUEXPAND: '.editor_mde_menu_expand'
    };
...
EditorPluginButtons.prototype = {
     ...
     * Possible states are:
     * <ul>
     * <li>{{#crossLink "M.editor_mde.EditorPluginButtons/ENABLED:property"}}{{/crossLink}}; and</li>
     * <li>{{#crossLink "M.editor_mde.EditorPluginButtons/DISABLED:property"}}{{/crossLink}}.</li>
     * </ul>
     *
     ...
     * See {{#crossLink "M.editor_mde.EditorSelection/selectionFilterMatches:method"}}{{/crossLink}} for more information.
    ...
    addButton: function(config) {
        var group = this.get('group'),
            ...
            buttonClass = 'mde_' + pluginname + '_button',
            button,
            host = this.get('host');
        ...
        var title = M.util.get_string(config.title, 'mde_' + pluginname);
        ...
        if (config.keys) {
            ...
            if (this._primaryKeyboardShortcut[buttonClass]) {
                ...
                title = M.util.get_string('plugin_title_shortcut', 'editor_mde', {
                    title: title,
        ...
        if (config.tags) {
            ...
            this._buttonHandlers.push(
                host.on(['mde:selectionchanged', 'change'], function(e) {
     ...
     *
     * See {{#crossLink "M.editor_mde.EditorPluginButtons/addButton:method"}}addButton{{/crossLink}}
    ...
    addToolbarMenu: function(config) {
        ...
        // Create the actual button.
        var id = 'mde_' + pluginname + '_menubutton_' + Y.stamp(this);
    ...
    _showToolbarMenu: function(e, config) {
        ...
        if (!this.menus[config.buttonClass]) {
            ...
            this.menus[config.buttonClass] = new Y.M.editor_mde.Menu(config);

            this.menus[config.buttonClass].get('contentBox').delegate('click',
                    this._chooseMenuItem, '.mde_menuentry a', this, config);
        }
     ...
     * @param {string} [keyConfig.container=.editor_mde_content] The containing element.
    ...
    _addKeyboardListener: function(callback, keyConfig, buttonName) {
        ...
        Y.log('mde shortcut registered: ' + keys + ' now triggers for ' + buttonName,
                'debug', LOGNAME);
    },
    ...
    _buttonHighlightToggled: function(buttonName, highlight) {
        ...
        if (toggledButton) {
            ...
            require(['editor_mde/events'], function(mdeEvents) {
                mdeEvents.notifyButtonHighlightToggled(toggledButton.getDOMNode(), buttonName, highlight);
            });
        }
    },
    ...
    _getDefaultMetaKeyDescription: function(keyCode) {
        if (Y.UA.os === 'macintosh') {
            return M.util.get_string('editor_command_keycode', 'editor_mde', String.fromCharCode(keyCode).toLowerCase());
        } else {
            return M.util.get_string('editor_control_keycode', 'editor_mde', String.fromCharCode(keyCode).toLowerCase());
        }
    },
...
Y.Base.mix(Y.M.editor_mde.EditorPlugin, [EditorPluginButtons]);
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
...
/**
 * Dialogue functions for an mde Plugin.
 *
 * See {{#crossLink "M.editor_mde.EditorPlugin"}}{{/crossLink}} for details.
 *
 * @namespace M.editor_mde
...
Y.Base.mix(Y.M.editor_mde.EditorPlugin, [EditorPluginDialogue]);
