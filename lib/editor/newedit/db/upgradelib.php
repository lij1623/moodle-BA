...
/**
 * Run all newedit upgrade steps between the current DB version and the current version on disk.
 * @param int $oldversion The old version of newedit in the DB.
 * @return bool
 */
function xmldb_editor_newedit_upgrade($oldversion) {
    global $CFG;
    ...
    if ($oldversion < 2019090900) {
        $toolbar = get_config('editor_newedit', 'toolbar');
        ...
        if (strpos($toolbar, 'h5p') === false) {
            ...
            foreach ($groups as $i => $group) {
                ...
                if (trim($parts[0]) == 'files') {
                    ...
                    set_config('toolbar', $toolbar, 'editor_newedit');
                }
            }
        }
        // newedit editor savepoint reached.
        upgrade_plugin_savepoint(true, 2019090900, 'editor', 'newedit');
    }
...
other = html';
        ...
        if (get_config('editor_newedit', 'toolbar') === $oldtoolbar) {
...
accessibility = accessibilitychecker, accessibilityhelper
other = html';
            set_config('toolbar', $newtoolbar, 'editor_newedit');
        }

        upgrade_plugin_savepoint(true, 2020052100, 'editor', 'newedit');
    }
...
other = html';
        ...
        // Check if the current toolbar config matches the old toolbar config.
        $sametoolbar = str_replace(["\r\n", "\r"], "\n", get_config('editor_newedit', 'toolbar')) == $oldtoolbar;
        // Check if the current showgroups config matches the old showgroups config.
        $sameshowgroups = get_config('newedit_collapse', 'showgroups') == 5;
...
other = html';
            set_config('toolbar', $newtoolbar, 'editor_newedit');
            set_config('showgroups', 6, 'newedit_collapse');
        }

        upgrade_plugin_savepoint(true, 2021062400, 'editor', 'newedit');
    }
