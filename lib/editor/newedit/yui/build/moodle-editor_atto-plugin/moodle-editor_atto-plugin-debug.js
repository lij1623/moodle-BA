YUI.add('moodle-editor_newedit-plugin', function (Y, NAME) {
...
/**
 * newedit editor plugin.
 *
 * @module moodle-editor_newedit-plugin
...
/**
 * A Plugin for the newedit Editor used in Moodle.
 *
 ...
 * @main
 * @constructor
 * @uses M.editor_newedit.EditorPluginButtons
 * @uses M.editor_newedit.EditorPluginDialogue
 */
...
var GROUPSELECTOR = '.newedit_group.',
    GROUP = '_group';
...
Y.extend(EditorPlugin, Y.Base, {
     ...
     * {{#crossLink "M.editor_newedit.EditorTextArea/updateOriginal"}}updateOriginal{{/crossLink}}.
     *
     * @method markUpdated
     */
    ...
    ATTRS: {
        ...
        group: {
            ...
            getter: function(groupName) {
                ...
                if (!group) {
                    group = Y.Node.create('<div class="newedit_group ' +
                            groupName + GROUP + '"></div>');
...
Y.namespace('M.editor_newedit').EditorPlugin = EditorPlugin;
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
...
/**
 * Button functions for an newedit Plugin.
 *
 * See {{#crossLink "M.editor_newedit.EditorPlugin"}}{{/crossLink}} for details.
 *
 * @namespace M.editor_newedit
...
var MENUTEMPLATE = '' +
        '<button class="{{buttonClass}} newedit_hasmenu" ' +
            'id="{{id}}" ' +
            'tabindex="-1" ' +
            'title="{{title}}" ' +
            'aria-label="{{title}}" ' +
            ...
            '<span class="editor_newedit_menu_icon"></span>' +
            '<span class="editor_newedit_menu_expand"></span>' +
        '</button>';

var DISABLED = 'disabled',
    HIGHLIGHT = 'highlight',
    LOGNAME = 'moodle-editor_newedit-editor-plugin',
    CSS = {
        EDITORWRAPPER: '.editor_newedit_content',
        MENUICON: '.editor_newedit_menu_icon',
        MENUEXPAND: '.editor_newedit_menu_expand'
    };
...
EditorPluginButtons.prototype = {
     ...
     * Possible states are:
     * <ul>
     * <li>{{#crossLink "M.editor_newedit.EditorPluginButtons/ENABLED:property"}}{{/crossLink}}; and</li>
     * <li>{{#crossLink "M.editor_newedit.EditorPluginButtons/DISABLED:property"}}{{/crossLink}}.</li>
     * </ul>
     *
     ...
     * See {{#crossLink "M.editor_newedit.EditorSelection/selectionFilterMatches:method"}}{{/crossLink}} for more information.
    ...
    addButton: function(config) {
        var group = this.get('group'),
            ...
            buttonClass = 'newedit_' + pluginname + '_button',
            button,
            host = this.get('host');
        ...
        var title = M.util.get_string(config.title, 'newedit_' + pluginname);
        ...
        if (config.keys) {
            ...
            if (this._primaryKeyboardShortcut[buttonClass]) {
                ...
                title = M.util.get_string('plugin_title_shortcut', 'editor_newedit', {
                    title: title,
        ...
        if (config.tags) {
            ...
            this._buttonHandlers.push(
                host.on(['newedit:selectionchanged', 'change'], function(e) {
     ...
     *
     * See {{#crossLink "M.editor_newedit.EditorPluginButtons/addButton:method"}}addButton{{/crossLink}}
    ...
    addBasicButton: function(config) {
        if (!config.exec) {
            Y.log('No exec command specified. Cannot proceed.',
                    'warn', 'moodle-editor_newedit-plugin');
            return null;
        }
    ...
    addToolbarMenu: function(config) {
        ...
        // Create the actual button.
        var id = 'newedit_' + pluginname + '_menubutton_' + Y.stamp(this);
    ...
    _showToolbarMenu: function(e, config) {
        ...
        if (!this.menus[config.buttonClass]) {
            ...
            this.menus[config.buttonClass] = new Y.M.editor_newedit.Menu(config);

            this.menus[config.buttonClass].get('contentBox').delegate('click',
                    this._chooseMenuItem, '.newedit_menuentry a', this, config);
        }
     ...
     * @param {string} [keyConfig.container=.editor_newedit_content] The containing element.
    ...
    _addKeyboardListener: function(callback, keyConfig, buttonName) {
        ...
        Y.log('newedit shortcut registered: ' + keys + ' now triggers for ' + buttonName,
                'debug', LOGNAME);
    },
    ...
    _buttonHighlightToggled: function(buttonName, highlight) {
        ...
        if (toggledButton) {
            ...
            require(['editor_newedit/events'], function(neweditEvents) {
                neweditEvents.notifyButtonHighlightToggled(toggledButton.getDOMNode(), buttonName, highlight);
            });
        }
    },
    ...
    _getDefaultMetaKeyDescription: function(keyCode) {
        if (Y.UA.os === 'macintosh') {
            return M.util.get_string('editor_command_keycode', 'editor_newedit', String.fromCharCode(keyCode).toLowerCase());
        } else {
            return M.util.get_string('editor_control_keycode', 'editor_newedit', String.fromCharCode(keyCode).toLowerCase());
        }
    },
...
Y.Base.mix(Y.M.editor_newedit.EditorPlugin, [EditorPluginButtons]);
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
...
/**
 * Dialogue functions for an newedit Plugin.
 *
 * See {{#crossLink "M.editor_newedit.EditorPlugin"}}{{/crossLink}} for details.
 *
 * @namespace M.editor_newedit
...
Y.Base.mix(Y.M.editor_newedit.EditorPlugin, [EditorPluginDialogue]);
