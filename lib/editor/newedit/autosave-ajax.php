...
$PAGE->set_url('/lib/editor/newedit/autosave-ajax.php');
...
if (!in_array('newedit', explode(',', get_config('core', 'texteditors')))) {
    print_error('accessdenied', 'admin');
}
...
foreach ($actions as $actionparams) {
    ...
    if ($action === 'save') {
        ...
        $record = $DB->get_record('editor_newedit_autosave', $params);
        ...
        if (!$record) {
            ...
            $DB->insert_record('editor_newedit_autosave', $record);
            ...
            $DB->update_record('editor_newedit_autosave', $record);
        ...
        if (!$record) {
            ...
            if (!$stale) {
                ...
                $DB->delete_records('editor_newedit_autosave', array('id' => $record->id));
        ...
        $DB->delete_records('editor_newedit_autosave', $params);
        $responses[] = null;
        continue;
    }
}
